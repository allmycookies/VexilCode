...
<div class="row">
 ...
 d-flex justify-content-between align-items-center <?php echo ($selectedProjectId === $project['id']) ? 'active' : '';
 ?>">
                            <a href="?tool=vergit&project_id=<?php echo $project['id'];
 ...
 ?>" data-project-name="<?php echo htmlspecialchars($project['name']); ?>" title="Projekt archivieren">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    <label for="vergit_storage_path" class="form-label">Aktueller Ablageort</label>
                    <div class="input-group">
                        <input type="text" class="form-control" name="vergit_storage_path" id="vergit_storage_path" value="<?php echo htmlspecialchars($storage_path); ?>" readonly>
                        <button class="btn btn-outline-secondary btn-folder-picker" type="button" data-target-input="#vergit_storage_path" disabled><i class="fas fa-folder-open"></i></button>
                    </div>
                    <div class="form-text">
                        Dieser Pfad wird in den allgemeinen <a href="?tool=settings">Einstellungen</a> konfiguriert.
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <?php foreach($archives as $archive): ?>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <div>
                             ...
                             <strong><?php echo htmlspecialchars($archive['original_project_data']['name']);
 ?></strong>
                                <small class="text-muted d-block">Archiviert am: <?php echo date('d.m.Y H:i', strtotime($archive['archived_at']));
 ?></small>
                            </div>
                            <div class="btn-group">
                     ...
                     <input type="hidden" name="archive_id" value="<?php echo $archive['id']; ?>">
                                ...
                                <form 
 method="post" action="?tool=vergit" class="d-inline">
                                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']);
 ?>">
                                    <input type="hidden" name="action" value="delete_archive">
                                    <input type="hidden" name="archive_id" value="<?php echo $archive['id'];
 ?>">
                                    <button type="submit" class="btn btn-sm btn-outline-danger" data-confirm="Soll das Archiv für '<?php echo htmlspecialchars($archive['original_project_data']['name']);
 ?>' wirklich ENDGÜLTIG gelöscht werden? Diese Aktion kann nicht rückgängig gemacht werden." title="Archiv endgültig löschen">
                    ...
                    <table class="table table-sm table-borderless">
                         ...
                         <tbody>
                            <tr>
                                ...
                                <td class="align-middle"><small class="text-muted"><?php echo 
 $lastVersion ? htmlspecialchars($lastVersion['version_number']) : 'N/A'; ?></small></td>
                                <td>
                                    <div class="input-group input-group-sm">
                         
                                 <input type="text" class="form-control" value="<?php echo htmlspecialchars($currentUrl['url']); ?>" readonly>
                                        <button class="btn btn-outline-secondary" type="button" onclick="navigator.clipboard.writeText(this.previousElementSibling.value)" <?php if (!$currentUrl['exists']) echo 'disabled';
 ?>><i class="fas fa-copy"></i></button>
                            ...
                            <tr>
                                <td class="fw-bold align-middle"><span class="badge bg-warning text-dark">Beta</span></td>
                                 ...
                                 <td class="align-middle"><small class="text-muted"><?php echo $betaVersion ?
 htmlspecialchars($betaVersion['version_number']) : 'N/A'; ?></small></td>
                                <td>
                                    <div class="input-group input-group-sm">
                           
                             <input type="text" class="form-control" value="<?php echo htmlspecialchars($betaUrl['url']); ?>" readonly>
                                        <button class="btn btn-outline-secondary" type="button" onclick="navigator.clipboard.writeText(this.previousElementSibling.value)" <?php if (!$betaUrl['exists']) echo 'disabled';
 ?>><i class="fas fa-copy"></i></button>
                                    </div>
                                </td>
 ...
 ?>
                            ...
                            <tr>
                                ...
                                <td class="align-middle"><small class="text-muted"><?php echo $stableVersion ?
 htmlspecialchars($stableVersion['version_number']) : 'N/A'; ?></small></td>
                                <td>
                                    <div class="input-group input-group-sm">
                           
                             <input type="text" class="form-control" value="<?php echo htmlspecialchars($stableUrl['url']); ?>" readonly>
                                        <button class="btn btn-outline-secondary" type="button" onclick="navigator.clipboard.writeText(this.previousElementSibling.value)" <?php if (!$stableUrl['exists']) echo 'disabled';
 ?>><i class="fas fa-copy"></i></button>
                                    </div>
                                </td>
 ...
 ?>
            ...
            <div class="card">
                <div class="card-header">
                    <h5><i class="fas fa-history me-2"></i>Versionen für: <?php echo htmlspecialchars($selectedProject['name']);
 ?></h5>
                </div>
                <div class="card-body">
 ...
 class="form-label">Neue Versionsnummer</label>
                            <input type="text" name="version_number" id="version_number" class="form-control" placeholder="z.B. 1.0.0, 1.1.0-alpha1, etc."
 required>
                            <?php if($lastVersion): ?><small class="form-text text-muted">Letzte Version war: <?php echo htmlspecialchars($lastVersion['version_number']);
 ?></small><?php endif; ?>
                        </div>
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="copy_last" id="copy_last" value="1" <?php echo $lastVersion ?
 '' : 'disabled'; ?>>
                    ...
                    <?php else: ?>
                        <?php foreach ($versions as $version): ?>
                            <li class="list-group-item">
                                    ...
                                    <div>
                                        <strong><?php 
 echo htmlspecialchars($version['version_number']); ?></strong>
 ...
 ?>
                                        <small class="text-muted d-block font-monospace"><?php echo htmlspecialchars($version['path']); ?></small>
                                        <small class="text-muted d-block">Erstellt: <?php echo $version['created_at'];
 ?></small>
                                    </div>
                                    <div class="btn-group">
                           ...
                           <li><button class="dropdown-item" type="button" onclick="submitVersionAction('set_channel_beta', '<?php echo $version['id']; ?>')"><i class="fas fa-flask fa-fw me-2"></i>Als Beta setzen</button></li>
                                         ...
                                         <li><hr class="dropdown-divider"></li>
                                            <li><button class="dropdown-item" type="button" data-bs-toggle="modal" data-bs-target="#releaseModal" data-version-id="<?php echo $version['id'];
 ?>" data-version-number="<?php echo htmlspecialchars($version['version_number']); ?>"><i class="fas fa-rocket fa-fw me-2"></i>Release erstellen...</button></li>
                                            <li><hr class="dropdown-divider"></li>
                                            <li><a class="dropdown-item" 
 href="?tool=filemanager&path=<?php echo urlencode($version['path']); ?>"><i class="fas fa-folder-open fa-fw me-2"></i>Dateimanager</a></li>
                                            <li><button class="dropdown-item" type="button" data-bs-toggle="modal" data-bs-target="#instanceModal" data-version-id="<?php echo $version['id'];
 ?>" data-version-number="<?php echo htmlspecialchars($version['version_number']); ?>"><i class="fas fa-server fa-fw me-2"></i>Instanzen</button></li>
                                            <li><button class="dropdown-item btn-show-diff" type="button" data-bs-toggle="collapse" data-bs-target="#diff-<?php echo $version['id'];
 ?>" data-version-path="<?php echo htmlspecialchars($version['path']); ?>"><i class="fas fa-exchange-alt fa-fw me-2"></i>Diff anzeigen</button></li>
                                            <li><hr class="dropdown-divider"></li>
                                            <li><button class="dropdown-item 
 text-danger" type="button" onclick="submitVersionAction('delete_version', '<?php echo $version['id']; ?>', 'Möchtest du die Version \'<?php echo htmlspecialchars($version['version_number']); ?>\' wirklich endgültig löschen?')"><i class="fas fa-trash-alt fa-fw me-2"></i>Löschen</button></li>
                                        </ul>
                                    </div>
   
                                     </div>
                                <div class="collapse mt-3" id="diff-<?php echo $version['id'];
 ?>"><div class="diff-container-wrapper p-3 bg-light border rounded"></div></div>
                            </li>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </ul>
             ...
             <div class="card mt-4">
                 ...
                 ?>
                     <li class="list-group-item text-muted">Keine Release-Archive für 
 dieses Projekt vorhanden.</li>
                 <?php else: ?>
                     <?php foreach ($releaseFiles as $file): 
                         $filePath = $releaseDir . DIRECTORY_SEPARATOR .
 $file;
 ...
 fa-fw text-secondary"></i>
                                 <strong class="font-monospace"><?php echo htmlspecialchars($file);
 ?></strong>
                                 <small class="text-muted d-block">Größe: <?php echo round(filesize($filePath) / 1024, 2);
 ?> KB</small>
                             </div>
                             <div class="btn-group">
                                 <a href="lib/file_manager_api.php?action=download&path=<?php echo urlencode($filePath); ?>" class="btn btn-sm 
 btn-outline-primary"><i class="fas fa-download me-1"></i>Download</a>
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
                <label for="release_name" class="form-label">Name des Archivs (ohne .zip)</label>
                     ...
                     <input type="text" name="release_name" id="release_name" class="form-control" placeholder="z.B.
 my-project-v1.0-final" required>
            </div>
          </div>
   ...
   <div class="modal-dialog modal-lg modal-dialog-scrollable">
      ...
      <div class="modal-body">
         <div class="d-flex justify-content-between align-items-center mb-3">
             <p class="mb-0 text-muted small">Verwalten Sie Test-Instanzen für diese Version.
 Jede Instanz ist eine eigenständige Kopie.</p>
...
<?php
// JS zum Befüllen des Release-Modals und Speichern der Projekt-ID
$script = <<<JS
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (selectedProjectId) {
        localStorage.setItem('vergit_last_project_id', selectedProjectId);
    }
});
JS;
