...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['sitemap_targetDirectory'])) {
    ...
    if (!is_dir($targetDirectory)) {
        $output .= "<span class='error'>[FEHLER] Das angegebene Verzeichnis '" .
 htmlspecialchars($targetDirectory) . "' existiert nicht.</span>\n";
    } elseif (!is_readable($targetDirectory)) {
        $output .= "<span class='error'>[FEHLER] Das angegebene Verzeichnis '" .
 htmlspecialchars($targetDirectory) . "' ist nicht lesbar (Prüfe Webserver-Berechtigungen!).</span>\n";
    } else {
...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card">
            ...
            <div class="card-body">
                <form method="get" action="?tool=sitemap" data-settings-form="sitemap" id="sitemap-form">
       
                     <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                    <input type="hidden" name="tool" value="sitemap">
                    <div class="input-group">
                         <input type="text" class="form-control form-control-lg" name="sitemap_targetDirectory" id="sitemap_targetDirectory" value="<?php echo htmlspecialchars($_GET['sitemap_targetDirectory'] ?? $settings['sitemap_targetDirectory']); ?>" 
 placeholder="Verzeichnis auswählen oder Pfad eingeben...">
 ...
 id="copySitemapBtn">
            ...
            <div class="card-body sitemap-output">
                <pre id="sitemapOutputContent"><?php echo $output;
 ?></pre>
            </div>
        </div>
        <?php endif;
