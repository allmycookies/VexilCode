...
if ($tool === 'editor') { include __DIR__ . "/tools/editor.php"; exit;
}
$logMessages = [];
...
if (file_exists($toolLogicFile)) { include_once $toolLogicFile; }
?>
<!DOCTYPE html>
<html lang="de">
<head>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top shadow-sm">
         <div class="container-fluid">
            <a class="navbar-brand" href="index.php"><i class="fas fa-toolbox"></i> VexilCode v0.9.4.b4</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#mainNav" aria-controls="mainNav" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
           </button>
            <div class="collapse navbar-collapse" id="mainNav">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <li class="nav-item"><a class="nav-link <?php echo $tool === 'filemanager' ? 'active' : ''; ?>" href="?tool=filemanager"><i class="fas fa-folder-tree fa-fw me-1"></i>Dateimanager</a></li>
                    <li class="nav-item"><a class="nav-link <?php echo $tool === 'vergit' ? 'active' : ''; ?>" href="?tool=vergit"><i class="fas fa-code-branch fa-fw me-1"></i>Vergit</a></li>
                    <li class="nav-item"><a class="nav-link <?php echo $tool === 'search_replace' ? 'active' : ''; ?>" href="?tool=search_replace"><i class="fas fa-search fa-fw me-1"></i>Suchen & Ersetzen</a></li>
                    <li class="nav-item"><a class="nav-link <?php echo $tool === 'collector' ? 'active' : ''; ?>" href="?tool=collector"><i class="fas fa-archive fa-fw me-1"></i>Collector</a></li>
                    <li class="nav-item"><a class="nav-link <?php echo $tool === 'disposer' ? 'active' : ''; ?>" href="?tool=disposer"><i class="fas fa-box-open fa-fw me-1"></i>Disposer</a></li>
                    <li class="nav-item"><a class="nav-link <?php echo $tool === 'sitemap' ? 'active' : ''; ?>" href="?tool=sitemap"><i class="fas fa-sitemap fa-fw me-1"></i>Sitemap</a></li>
                    <li class="nav-item"><a class="nav-link <?php echo $tool === 'settings' ? 'active' : ''; ?>" href="?tool=settings"><i class="fas fa-cogs fa-fw me-1"></i>Einstellungen</a></li>
                </ul>
    ...
    <main class="container-fluid mt-4 mb-4">
       ...
       <?php
        ...
        if (isset($_SESSION['flash_success'])) {
            echo '<div class="alert alert-success alert-dismissible fade show" role="alert">' . htmlspecialchars($_SESSION['flash_success']) . 
                 '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
            unset($_SESSION['flash_success']);
        }
         // Flash-Nachrichten für Vergit-Fehler anzeigen
        if (isset($_SESSION['flash_error'])) {
            echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">' . htmlspecialchars($_SESSION['flash_error']) . 
                 '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
            unset($_SESSION['flash_error']);
        }
         
        if (function_exists('renderToolUI')) { renderToolUI($settings, $logMessages);
        } else { echo '<div class="alert alert-danger">Tool-UI-Funktion nicht gefunden.</div>'; } 
        ?>
    </main>
                    ...
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Abbrechen</button>
                     <div>
                        <button type="button" class="btn btn-primary" id="fm-unzip-to-folder-btn">In neuen Ordner</button>
                          ...
                          <button type="button" class="btn btn-info" id="fm-unzip-here-btn">Hier entpacken</button>
                    </div>
                 </div>
            </div>
        </div>
